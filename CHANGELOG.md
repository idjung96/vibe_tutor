...
## [1.73.0] — 해제 목록이 템플릿의 **예시**를 실제 해제로 읽던 것

실제 프로젝트(`flowlab_in_kims`)에 v1.72.0 을 설치해 확인하다 나왔다. 설치 자체는 깨끗했다
(검증 15/15, 게이트 통과, 가드 훅 30/30, `guards/` 안 덮임, 역할 도구에 Grep 반영).

그런데 해제 목록을 뜯어보니 이렇게 인식되고 있었다:

```
해제로 인식된 것: ['api/tests/test_contract.py', 'tests/conftest.py',
                  'tests/test_check_steps.py', 'tests/test_payment.py']
                                               ^^^^^^^^^^^^^^^^^^^^^^^ 템플릿의 형식 예시
```

`TEST_UNFREEZE.md` 템플릿은 형식을 코드블록 안 예시로 보여 준다. 파서가 코드블록을 모르고
그 줄을 **실제 해제 항목으로 읽었다.** 그래서 갓 설치한 프로젝트는 `tests/test_payment.py`
를 **처음부터 풀린 채로** 갖는다 — 그 경로에 테스트를 만들어 커밋해도 가드가 안 막는다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 코드블록 안 예시 | **실제 해제로 읽음** | 건너뛴다 |
| 갓 설치한 프로젝트 | `tests/test_payment.py` 가 풀려 있음 | 해제 0건 |

`--migrate-check` 가 템플릿의 형식 예시를 유령 절로 세던 것과 **같은 유형**이다 —
설명하려고 적은 예시를 데이터로 읽는 것. `guard.js` 도 함께 고쳤다(세 경로 동기화).
`verify_hooks.sh` 31항목.

덧붙여, 이번 확인에서 내가 한 번 틀렸다. `tests/conftest.py` 가 통과하는 것을 보고 결함이라
판단했는데, **그 프로젝트의 해제 목록에 근거와 함께 올라가 있었다** — 가드는 맞게 동작했다.
디버그 과정에서 훅을 `/tmp` 에 복사해 돌린 것도 틀렸다(`ROOT` 가 `/` 로 잡혀 fail-closed 가
떠서 엉뚱한 결론이 나올 뻔했다). 훅은 자기 위치로 루트를 잡으므로 **설치된 자리에서** 돌려야 한다.
...
넷은 전부 Grep 을 가졌다 — 설계 의도를 만들기 어렵다, 누락으로 보인다." 맞다. 그리고 재 보니
**tester 만의 문제가 아니었다.**

| 역할 | 지금(AS-IS) | 실제 의미 |
|---|---|---|
| tester · designer | `Read, Write` | **탐색 수단이 아예 없다** |
| coder · checker · documenter | `…, Bash`(Grep 없음) | Grep 대신 **Bash 로 grep 한다** |
| planner · reviewer · lead · critic · security | `…, Grep` | 정상 |

**Grep 은 Read 이상의 권한을 주지 않는다.** 모든 역할이 이미 Read 를 갖고 있으니, 빼 봐야
권한이 줄지 않고 둘 중 하나가 된다:

- **역할이 제 일을 못 한다.** `tester` 는 규칙 7 "기존 테스트 파일은 절대 수정하지 않는다" 를
  지켜야 하는데 **무엇이 기존인지 찾을 수단이 없었다.** 파일명 관례도 기존 테스트를 봐야 맞출
  수 있다. `designer` 도 같다.
- **더 큰 권한으로 우회한다.** `coder`·`checker`·`documenter` 는 Grep 없이 Bash 를 가져서
  `bash grep` 을 쓴다 — **작은 도구를 막고 큰 도구를 열어 둔 꼴**이다.

게다가 이 제한은 **Claude 에만** 걸렸다. opencode 템플릿은 `write/edit/bash` 만 적어
read·grep 을 제한하지 않고, codex 역할에는 도구 제한이 아예 없다. **같은 역할이 에이전트마다
다르게 동작**했으니 이 저장소의 "산출물의 비종속성" 과 어긋난다.

그래서 이건 권한을 새로 주는 결정(C등급)이 아니라 **망가진 정의를 복원하는 버그 수정**이다.
Read 를 가진 역할 전부에 Grep 을 준다. 쓰기·실행 권한은 한 글자도 안 바뀐다.

`verify_parity.py` 에 **"Read 를 가진 역할은 Grep 도 갖는다"** 를 넣었다(25항목).
표를 눈으로 보다 놓친 누락이라 구조로 고정한다 — tester 에서 Grep 을 빼 실제로 잡히는지 확인했다.

## [1.70.0] — 해제 목록이 절대경로에서 통째로 안 먹던 것

사용처가 D-232 로 올린 관찰이다. 내가 v1.67.0 에 넣은 버그이고, **해제 목록이 실질적으로
동작하지 않는 상태**였다.

```
해제 목록에 tests/test_a.py 를 「근거:」와 함께 적어 두고
  상대경로로 편집 -> rc=0  (통과)
  절대경로로 편집 -> rc=2  (차단)   ← 같은 파일인데
```

**편집 도구는 보통 절대경로를 넘긴다**(Claude Code 의 Edit 이 그렇다). 해제 목록은 사람이
쓰므로 상대경로다. 둘을 문자열로 비교해서, 같은 파일의 표기 셋(절대·상대·`./상대`)이 갈렸다.
그래서 "해제했는데 안 풀리는" 상태가 된다 — 목록이 있으나 마나였다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 경로 비교 | 문자열 그대로 | **루트 기준 상대경로로 맞춰서** |
| 절대경로 + 해제됨 | **차단**(버그) | 통과 |
| 절대경로 + 해제 안 됨 | 차단 | 차단(그대로) |

`guard.js` 도 같은 규칙으로 고쳤다(세 경로 동기화). `verify_hooks.sh` 에 회귀를 넣었다 —
세 표기 × 해제/비해제를 전부 본다(30항목).

### 확장점이 차단을 되돌릴 수 없다는 지적 — 설계가 맞고, 문서가 없던 것이 문제다

훅이 차단을 정하면 그 자리에서 `exit 2` 하므로 `guards/project.sh` 는 **불리지도 않는다.**
확장은 **조일 수만 있고 풀 수는 없다.** 이건 의도한 방향이다 — 안전장치의 확장점이 차단을
풀 수 있으면 그 확장점이 곧 우회로가 된다.

**푸는 경로는 `TEST_UNFREEZE.md`** 다. 그 파일도 프로젝트 소유이고 업그레이드가 안 덮는다.
이번에 그게 실제로 동작하게 됐으니, 하니스 파일을 고칠 이유도 없어진다 — D-232 의 시한부
성격이 사라진다. 그 방향을 `guards/README.md` 와 `CLAUDE.md` 에 명시했다.

## [1.69.0] — 절차 무결성 감사, 그리고 고아가 된 단계 둘

"개발 프로세스·에이전트·협업·자산 관리를 리뷰해서 이 하네스가 제대로 동작할지 확인해" 라는
Owner 요청. 눈으로 보지 않고 기계로 훑었다.

**멀쩡한 것**: selfcheck 플래그 13종이 전부 구현돼 있고, 설치되는 역할 10종이 전부 절차에서
불리며(그 반대도), 가리키는 단계 번호가 전부 실재한다. 자산 경로도 이어져 있다 —
`lib-research` → `dev-agent-team/libs/<이름>.md` + `INDEX.md`, coder·security 가 읽는다.

**고장 난 것 — 끼워 넣은 단계 둘이 고아였다.**

| 어디 | 지금(AS-IS) | 실제로 벌어지는 일 |
|---|---|---|
| Gate 0(3번) | `1: 계획으로 넘어간다(4번)` | **4-0(lead 방향 설정)을 건너뛴다** |
| 단계 merge(12번) | `12c로.` | **12b-1(완료 백로그 아카이브)을 건너뛴다** |

둘 다 이번 세션에 내가 단계를 끼워 넣고 전이를 안 고친 것이다. **12b-1 이 막으려던 것이
바로 "백로그가 쌓이기만 하는 것"** 이라, 고아가 된 순간 그 문제가 조용히 계속된다 —
장치를 넣고 안 돌리는 것이 제일 나쁜 모양이다.

`tests/test_procedure.py` 를 만들어 `run_all.sh` 에 넣었다(6항목, 전체 12/12).
세 종류 결함을 주입해 실제로 잡히는지 확인했다 — 고아 단계·없는 플래그·없는 역할.

만들면서 두 가지를 배웠다.
- **점프는 두 종류다.** "…없으면 7번으로 간다"(조건부)는 다음 단계를 건너뛰지 않는다.
  안 가르면 멀쩡한 단계가 고아로 찍히고 **오탐이 나는 검사는 곧 무시된다.**
- **한국어에서 파이썬 `\b` 는 경계를 안 만든다**(한글이 `\w` 다). `\bcritic\b` 가 `critic을` 에
  안 맞아 이 감사를 짜면서 세 번 헛돌았다. 라틴 문자 경계로 판정한다.

**남은 위험은 부하다.** 이번 세션에 SKILL 377→536줄(+42%), selfcheck 1186→2004줄(+69%),
단계 하나에 번호 17개다. 이 저장소의 교훈이 "절차 문장은 지켜지지 않는다" 인데 절차가 길어졌다.
다만 새로 붙은 6개(7-0b·7-0c·12b-1·12c-1·12c-2·12c-3) 중 넷은 기계 판정이 YES 일 때만 돌고,
12c 는 LLM 회고를 **줄이는** 장치다. 순조로운 단계의 실제 부하는 그대로다.

## [1.68.0] — 개발 상태를 GitHub 이슈로 추적하지 않는다

"하네스가 모든 이슈를 등록하고 개발이 끝나면 close 하는가" 라는 Owner 질문에서 사실을 확인했다.
**아니다** — `gh issue create` 가 절차에 **딱 한 군데**(결정의 조사 첨부) 있었고,
`close` 는 **어디에도 없었다**. 그 상태를 Owner 가 판정했다: 지금 구조에서 git 으로 개발
상태를 추적하는 것은 적절하지 않다.

그러면 **만들라고 시키는 것 자체가 틀린 기본값**이다. 기본을 파일로 되돌린다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 조사 기록 | remote 있으면 **이슈를 만들라** | `dev-agent-team/evidence/` 로 뺀다 |
| 이슈 | 절차가 생성을 지시 | **하네스는 만들지도 닫지도 추적하지도 않는다** |
| 프로젝트가 이미 쓰면 | — | `조사: #번호` 로 적어도 된다. 여닫는 것은 프로젝트 몫 |

이유 셋을 문서에 박았다.
- **상태는 전부 파일에 있어야 한다.** small↔large 무손실 전환의 전제다. 상태가 원격에
  나뉘면 프로파일을 바꾸거나 저장소를 옮길 때 절반이 안 따라온다.
- **small 프로파일에는 remote 자체가 없다**(온프레미스·오프라인). 절차가 갈리면 안 된다.
- **만드는 규칙만 있고 닫는 규칙이 없으면 샌다.** 실측: 이슈를 참조한 커밋 216개 vs
  닫는다고 적은 커밋 4개. 이 저장소가 계속 고쳐 온 "쌓이기만 하는 것" 을 또 만들지 않는다.

push·PR(main 합류 지점)은 그대로 둔다 — 그건 상태 추적이 아니라 **코드 전달**이고 이미
remote 조건부다.

## [1.67.0] — 업그레이드가 프로젝트 안전장치를 두 번 지운 사고

한 사용처에서 1.51→1.58, 1.58→1.66 **두 번 연속** 프로젝트가 넣은 집행 장치가 사라졌다.
두 번째가 핵심이다 — 첫 사고 뒤 「집행 표식이 파일 안에 있는지 게이트가 확인한다」는 검사를
`selfcheck.py` 에 넣었는데, **그 검사까지 함께 지워졌다.** 그래서 게이트는 `[guards] OK` 를
찍었다. **탐지기가 탐지 대상과 함께 사라지면 침묵한다.**

### ① 확장점 — 하니스를 고치지 않고 덧댈 수 있다

`hooks/*.sh` 와 `selfcheck.py` 를 무조건 덮는 것 자체는 맞다(옛 강제 장치에는 구멍이 있다).
틀린 것은 **덧댈 자리가 없었다**는 것이다. 확장점이 없으면 사용처의 합리적 반응이
"업그레이드를 안 하는 것" 이 되고, 그러면 새 기능이 영영 전달되지 않는다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 프로젝트 규칙 | 하니스 파일에 직접 → **업그레이드마다 소실** | `dev-agent-team/guards/` — 하니스가 덮지 않는다 |
| 가드 훅 확장 | 없음 | `guards/project.sh` — `$HARNESS_CANDIDATES` 받고 `exit 2` |
| 검사 축 확장 | 없음 | `guards/project_checks.py` — `run(ctx)` 가 축 목록 반환 |
| 확장이 터지면 | — | **막는다**(fail-closed). 못 돌린 확장을 "통과" 로 바꾸지 않는다 |

`/etc/profile ↔ ~/.bashrc`·`nginx conf.d/`·`git core.hooksPath` 와 같은 자리다.

### ② 매니페스트를 실제 교체 대상까지

매니페스트에 `AGENTS.md`·`CLAUDE.md` 두 줄뿐이라 **정작 갈아엎히는 파일이 관리 목록 밖**이었다.
이제 `hooks/*.sh`·`selfcheck.py` 도 해시를 기록하고, 프로젝트가 고친 흔적이 있으면 덮되
**`.orig` 로 남기고 크게 알린다** — 무엇을 잃었는지 `diff` 로 보고 guards/ 로 옮기라고 말한다.
(`--migrate-check` 는 문서 규약을 보는 것이라 이 사고를 못 잡는다.)

### ③ 인자를 여럿 받는다 (두 번 고쳤다 두 번 되돌아온 버그)

`_parse_args` 가 `rest[0]` 만 돌려줬다. `selfcheck.py A.py B.py` 가 A.py 만 보고 **"0건"** 을
찍는다 — **검사했는데 깨끗한 것과 아예 안 본 것이 같은 글자**다. `[collect]` 축도 pytest 에
첫 파일만 넘겼고, 그건 merge 를 막는 결정적 축이라 더 무겁다.

- `_parse_args` 가 리스트를 돌려준다(플래그의 **값**은 대상에서 제외).
- `collect_tests` 가 `cmd.extend(targets)`.
- 축 출력에 **실제로 본 파일 수**를 붙인다 — `[print] 0건 (2파일 검사)`.
- 못 읽은 파일은 「0건」에 섞지 않고 따로 말한다 — `… 못 읽은 파일 1개는 제외 — [read] 참조`.

### ④ 「기존 테스트」의 판정축을 git 추적 여부로

헌법 2번의 「기존」은 커밋된 것을 뜻하는데 훅은 **파일 존재**로 판정했다. 그래서 에이전트가
방금 만들어 아직 커밋도 안 한 테스트를 스스로 못 고쳤다 — 한 프로젝트에서 여덟 번 재발했고,
그때마다 자기 산출물의 결함을 발견하고도 못 고친 채 넘어갔다(한 번은 실제 구멍이 통과했다).

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 판정축 | 파일 존재 | **git 추적 여부** |
| 미커밋 테스트 | 차단(자기 산출물을 못 고침) | **통과** |
| 커밋된 테스트 | 차단 | 차단 — 단 `TEST_UNFREEZE.md` 에 **「근거:」와 함께** 적힌 경로는 통과 |
| git 없음·판정 실패 | — | **차단**(fail-closed). 해제 목록을 못 읽어도 전면 동결 |
| 보호 범위 | `test_*.py`·`*_test.py` | `tests?/` 아래 **모든 .py**(conftest·_contract·_synthetic 포함) |

훅은 프로젝트 루트를 **자기 위치**로 안다(cwd 가 아니다) — 하위 폴더에서 불려도 git 판정과
해제 목록 경로가 어긋나지 않는다. `guard.js` 도 같은 규칙으로 포팅했다(세 경로 동기화).

`verify_hooks.sh` 는 샌드박스를 **진짜 git 저장소**로 만들고 「기존」 픽스처를 커밋한다 —
판정축이 바뀌었으니 테스트 전제도 바뀌어야 한다. 26 → **29항목**.
`test_selfcheck.py` 169항목, `verify_parity.py` 24항목.

## [1.66.0] — PROCESS 순증에 값을 붙인다

v1.65.0 리뷰에서 남긴 병목을 잡는다. PROCESS 가 모든 역할 호출에 988줄(coder 1247)씩 붙는데
**주입 예산을 걸 수 없다** — 로그는 접혀도 이력이 안 보일 뿐이지만 운영 규칙을 조용히 빼면
그 규칙이 안 지켜진다(원칙 1).

원천에서 줄이는 수밖에 없는데, 실측 프로젝트에서 규칙이 76개까지 자라는 동안 통합·폐기는
**2번**뿐이었다. 왜인가 — **통합이 "또 하나의 IMPROVE" 라서 새 규칙 추가와 경쟁했고,
트리거도 목표도 없었다.** 기록해 둔 "폐기는 새 항목으로 표현한다" 는 절차 문장이었다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 통합 시점 | 없음(권고만) | **기계 트리거** — 가장 무거운 역할의 주입량이 권고 초과 |
| 새 규칙 추가 | 언제나 자유 | **순증 금지**(7-0c) — 초과 상태면 최소 하나를 대체해야 한다 |
| 무엇을 줄일지 | 알아서 | **좁히기 후보를 짚는다** — `대상: 전체` 인데 본문에 역할이 하나뿐인 블록 |
| 줄었는지 | 안 봄 | 7-0b e-2 에서 다시 재고 안 줄었으면 안 된 것 |

**순증 금지가 핵심이다.** 규칙을 못 만들게 막는 것이 아니라 **값을 치르게** 한다 —
WIP 제한과 같은 원리다. 자유롭게 늘 수 있으면 늘기만 한다는 것을 76:2 가 보여 줬다.

**좁히기는 즉효다.** `대상: 전체` 는 아홉 역할에 전부 붙으므로 하나만 좁혀도 크게 준다.
실측에서 전체 46개 중 **10개(270줄)** 가 본문에 역할이 하나만 나왔다 — P-64는 reviewer,
P-38은 tester, P-35는 lead 얘기인데 전체로 붙어 있었다. 역할이 여럿 나오면 후보로 올리지
않는다(자유 텍스트로 추측하지 않는다 — 원칙 2).

`tests/test_selfcheck.py` 10항목 추가(160항목).

## [1.65.0] — 하네스를 올린 뒤 기존 내용을 어떻게 할 것인가

Owner 요구 둘. **(1) 컨텍스트가 실제로 짧아졌는지 리뷰 · (2) 하네스 업데이트 때 백로그와
생성된 내용을 마이그레이션할 방법.**

### (1) 리뷰 — 역할 하나를 부를 때의 총량을 처음 쟀다

| 역할 | 전 | 후 | 남은 최대 항목 |
|---|---|---|---|
| lead | 8859 | **2526** | PROCESS 988 + BACKLOG 915 |
| critic | 7797 | **1566** | PROCESS 988 |
| designer | 6744 | **1440** | PROCESS 988 |
| coder | 2777 | **2353** | **PROCESS 1247**(합계의 53%) |
| checker·reviewer·security | 1869 | **1183** | PROCESS 988 |

**남은 병목은 도구로 못 줄인다.** PROCESS 에는 예산을 걸 수 없다 — 로그는 접혀도 이력이
안 보일 뿐이지만 **운영 규칙을 조용히 빼면 그 규칙이 안 지켜진다**(원칙 1). 원천에서
통합해야 하고, BACKLOG 도 7-0b 폐기가 돌아야 준다. **둘 다 아직 한 번도 안 돌았다.**

### (2) 마이그레이션 — `selfcheck.py --migrate-check`

새 규약은 앞으로 쓸 것에만 적용된다. 이미 쓴 것이 규약을 안 따르면 **도구가 덜 먹는다** —
단계 번호가 없으면 아카이브가 안 되고, `[규칙]` 이 없으면 지켜야 할 것이 로그와 함께 내려간다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 업데이트 후 기존 내용 | 아무도 안 본다 | `--migrate-check` 가 센다 |
| 자동 변환 | — | **하지 않는다.** Owner 의 글이다 |
| 언제 보이나 | — | `init.sh` 가 설치 끝에 종수만 알린다 |

실측 11종: 단계 번호 없는 절(DECISIONS 43·DIRECTION 31·DESIGN 7) · 고정 절 0개 ·
40줄 넘는 절 26개 · PROCESS 주입 1305줄 · 완료 미이관 5건 · 묵은 항목 139건.

**자동으로 고치지 않는 이유는 이 저장소가 이미 데었기 때문이다** — v1.53.0 에서 재설치
두 번에 Owner 의 157줄짜리 헌법이 사라졌다. 설치기가 Owner 의 글을 고쳐 쓰는 길은 다시 안 간다.

만들면서 오탐 둘을 잡았다. 템플릿의 형식 예시(`## [날짜] 단계N: 제목`)가 **유령 절**로
집계됐다 — 파서가 코드블록 안의 `##` 를 세지 않게 고치고 템플릿을 펜스로 감쌌다.
그리고 PROCESS 의 P-규칙에 "단계 번호를 붙여라" 라고 했다 — **PROCESS 는 단계 로그가 아니라
규칙 집합**이다. 문서 종류를 가르는 일을 이번 대화에서 세 번째로 했다.

그리고 `init.sh` 가 `set -u` 에서 죽는 것을 run_all 이 잡았다 — `PY_BIN` 을
`--accept-constitution` 분기 안에서만 정의해 두고 설치 끝에서 썼다. 설치 검증은 이미 통과한
뒤라 **화면상으로는 성공처럼 보였다.** 정의를 한 곳으로 올리고 회귀 테스트를 넣었다.

`tests/test_selfcheck.py` 150항목 · `tests/test_install.py` 116항목.

## [1.64.0] — 지켜야 할 규칙은 로그와 함께 내려가지 않는다

"타이포 규칙은 지켜야 할 내용이다. 로그로 갈 내용이 아니다" 라는 Owner 지적.
맞고, **v1.63.0 의 장치에 구멍이 있었다.**

"승격 먼저, 아카이브 나중" 이라고 절차에 적어 두었을 뿐이라, `--ledger-archive` 는 규칙이
든 절도 그냥 내려버린다. 이 저장소가 이미 배운 것이다 — **절차 문장은 지켜지지 않는다.**
P-규칙 통합도 "폐기는 새 항목으로 표현한다" 고 적어 두었지만 76개 중 2번만 일어났다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 규칙을 지키는 방법 | 절차 문장("승격을 먼저 하라") | **구조** — `## [규칙] 제목` 절 |
| 그 절의 주입 | 오래되면 접힌다 | **접히지 않는다.** 본문째 들어간다 |
| 그 절의 아카이브 | 단계 번호가 있으면 내려간다 | **내려가지 않는다** |
| 아카이브가 옮긴 것 | 건수만 | **제목을 보여 준다** — 섞여 든 규칙을 되돌릴 수 있게 |

판정은 **쓰는 사람이 단 표시**로 한다. 자유 텍스트에서 "전역"·"규칙" 같은 낱말을 찾아
추측하지 않는다(원칙 2). 표시가 없으면 로그로 본다.

실측 근거가 바로 그 타이포 규칙이다 — `§121 폰트 크기 토큰 체계(AppFontSize, 전역 타이포
정리)` 80줄이 **단계121 절에** 들어 있었다. 전역 규칙인데 단계에 묶여 있어서, 그 단계가
아카이브되는 순간 다음 designer 가 토큰 체계를 못 본다. 이제 그 절 제목에 `[규칙]` 만
달면 영구히 전달된다.

`tests/test_selfcheck.py` 10항목 추가(141항목) — 단계 번호가 붙은 고정 절이 아카이브에서도
주입에서도 살아남는지 확인한다.

## [1.63.0] — DESIGN 도 규칙과 아카이브로 가른다

"디자인도 rules 와 archive 로 정리할 수 없나" 라는 Owner 질문. v1.62.0 에서 나는 DESIGN 을
"명세라 축이 다르다"며 제외했는데, **실물을 보니 그 전제가 틀렸다.**

```
DESIGN.md 24개 절 중 19개가 단계에 묶여 있다
  단계10 · 단계15 · 단계28 … §161 · §175 · 단계196~199 · stage-202
```

명세가 아니라 **"그 단계에 무슨 화면을 어떻게 만들었나" 의 로그**다. 그래서 DECISIONS 와
같은 규칙을 그대로 적용한다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| designer 가 받는 DESIGN | **전문 4880줄** | `--ledger DESIGN.md` — 예산 400줄 안 |
| 파일 크기 | 안 줄어든다 | `--ledger-archive DESIGN.md` 로 옮긴다 |
| 고정으로 남길 것 | 섞여 있다 | **머리말**(첫 `##` 앞) — 접히지도 아카이브되지도 않는다 |

**고정을 둘 곳이 DECISIONS 와 다르다.** 결정의 영속 제약은 `PROJECT_RULES.md` 로 승격하는데,
그건 모든 역할에 간다. 디자인 토큰 체계를 거기 넣으면 coder·checker·tester 호출까지 무거워진다.
그래서 디자인 규칙은 **`DESIGN.md` 머리말**에 둔다 — designer 에게만, 언제나 통째로.
실측에 `§121 폰트 크기 토큰 체계(AppFontSize, 전역 타이포 정리)` 가 단계 절에 들어 있었다.
그 단계가 아카이브되면 다음 designer 가 토큰 체계를 못 본다.

**잣대도 갈랐다.** DESIGN 을 ledger 로 바꾸자 "40줄 넘는 절 = 조사가 섞였다" 경고가 거기까지
갔다 — 화면 명세 500줄은 조사가 아니라 정상이다. `SECTION_TERSE`(결정·개정·방향)에만
그 잣대를 댄다. 전부에 같은 경고를 울리면 또 늘 울린다.

덤으로 **단계 번호 없는 절**을 센다 — 아카이브되지 않아 영영 쌓인다(DECISIONS 43개·DESIGN 7개).
그리고 머리말이 예산의 40%를 넘으면 알린다(머리말도 무한정 자랄 수 있다).

`tests/test_selfcheck.py` 9항목 추가(131항목).

## [1.62.0] — 주입에 상한을 두고, 파일을 실제로 줄인다

Owner 요구가 세 줄로 정리됐다. **파일이 무진장 커지지 않을 것 · 주입이 될 것 ·
주입이 동작하려면 컨텍스트가 너무 길지 않을 것.** v1.60.0 은 세 번째를 못 지키고 있었다.

제목 인덱스를 전부 주고 있었기 때문이다. 결정이 늘면 인덱스가 같이 늘어 **주입이 O(n)** 이다:

```
--ledger 출력 634줄 중 제목 인덱스가 337줄(53%)
결정 208건 → 505줄 · 400건 → 697줄 · 800건 → 1097줄
```

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 주입 크기 | **O(n)** — 제목이 전부 들어간다 | **예산 400줄 상한.** 최근 본문 → 제목 → 접기 |
| 접힌 구간 | — | "그 이전 N건이 있다" **한 줄로 알린다.** 숨기지 않는다 |
| 절 하나가 1447줄이면 | 통째로 들어간다(상한 무의미) | **잘라서 준다** + 잘랐다는 사실과 원문 위치 |
| 파일 크기 | 안 줄어든다 | **`--ledger-archive`** 로 `_ARCHIVE.md` 에 **옮긴다** |
| 오래된 유효 제약 | 창 밖으로 나가 묻힌다 | **승격 먼저**(`PROJECT_RULES.md`), 아카이브 나중 |

**승격과 아카이브의 순서가 이 설계의 급소다.** "오래됐다"와 "안 중요하다"는 다르다.
`우리는 sqlite 를 쓴다`, `argonet 저장소에 push 하지 않는다` 같은 제약은 몇 백 단계 전
결정이어도 지금 유효하다. 그런 것을 로그에 둔 채 아카이브하면 팀이 모르고 어긴다 —
못 본 것을 "제약 없음" 으로 바꾸는 것이라 원칙 1 위반이다. 그래서 7-0b 에서 먼저 가른다:
**"이걸 모르고 작업하면 팀이 틀리게 가는가?"** 그렇다면 규칙이고, `PROJECT_RULES.md` 는
언제나 통째로 전달된다. 접히는 것은 지나간 로그뿐이어야 한다.

실측(DECISIONS 5424줄 208건): 아카이브(keep 10) 뒤 **파일 2534줄 · 주입 122줄**,
절 유실 0, 멱등.

작업 중 제 테스트가 이 저장소 원칙 2를 그대로 어긴 것도 잡았다 — `"본문 3" not in out` 이
꼬리말의 `본문 3건` 에 걸렸다. 꼬리말 낱말을 `전문=3개` 로 바꾸고 판정을 구조로 고쳤다.

`tests/test_selfcheck.py` 15항목 추가(122항목).

## [1.61.0] — 결정은 결정만, 백로그는 버리는 단계를 둔다

"요구사항·결정·히스토리 파일이 커진다. 작게 유지하려면? 그냥 코드를 읽는 게 낫나" 라는
Owner 질문. 재 보니 **항목 수가 아니라 한 항목의 크기**가 문제였다.

```
DECISIONS 208건 5421줄 · 중앙값 7줄 · 평균 26줄 · 최대 1447줄
  41줄 넘는 26건(12%)이 전체의 63%
```

템플릿이 정한 형식은 `- 선택: 한 줄` 이고 **중앙값 7줄은 그걸 지키고 있었다.** 안 지킨 26건이
파일의 63%다. 1447줄짜리를 열어 보니 앞 30줄이 결정이고 나머지는 조사 기록이었다. 그 안에
`payment_screen.dart:431` 같은 위치 증거가 박혀 있었다 — **코드가 한 줄만 움직여도 거짓말이
되는 것**이다. "그냥 코드를 읽는 게 낫나"에 대한 답이 여기 있다: 코드로 알 수 있는 것은
코드로 알아내야 하고, 문서에는 코드가 절대 말해 주지 않는 것만 남긴다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 결정 한 건 | 제한 없음 | 등급·질문·선택·이유·**버린 대안**·**되돌리기**·조사 링크, 15줄 목표 |
| 구현 설명·`file:431` | 본문에 박음 | **적지 않는다.** 필요하면 커밋 해시 |
| 조사 과정 | 결정 안에 통째로 | **GitHub 이슈**(remote 있으면) 또는 `evidence/`. 번호만 남긴다 |
| 이슈가 안 읽히면 | — | 결정은 파일 안에 온전하다. 이슈가 사라져 곤란하면 그건 결정이다 |
| 백로그 상태 | `[ ]`/`[x]` | `[~] · 진행:stageN` 추가. **파일을 셋으로 쪼개지 않는다**(WIP=1) |
| 묵은 항목 | 아무도 안 버림 | **7-0b 백로그 정리** — 조건부 별도 단계 |
| 폐기 | — | 삭제가 아니라 `BACKLOG_DONE.md` 로 이동. 사유 4종 중 하나. Owner 항목은 C등급 |

칸반 세 컬럼을 파일로 만드는 안은 기각했다. 단계를 하나씩 도므로 **WIP 가 이미 1**이고,
todo/done 은 이미 나뉘어 있다. 모자란 것은 컬럼이 아니라 진행 표시와 계측이었다.
그리고 열린 항목 279개 중 139개가 20단계 넘게 묵은 것은 **컬럼이 없어서가 아니라 버리는
결정을 아무도 안 해서**다 — 그루밍이 7-0(방향 설정)에 묶여 있어 관심이 단계 초점으로 갔다.

`--ledger-stats` 는 이제 **문서 종류마다 다른 잣대**를 댄다. 처음엔 "40줄 넘는 절" 경고를
전부에 걸었더니 BACKLOG(목록)·DESIGN(명세)까지 걸렸다 — 그 둘은 긴 게 정상이다.
늘 울리는 경보는 무시되고, 무시되는 검사는 장식이다.

`BACKLOG_DONE.md`·GitHub 이슈(45건)·`evidence/` 는 모두 **하네스에 없는데 팀이 스스로 만들어
쓰던 것**이다. 세 번 나왔으면 하네스가 모자란 것이라 전부 정식화했다.

`tests/test_selfcheck.py` 10항목 추가(107항목).

## [1.60.0] — 긴 이력을 자르지 않고 나눈다 (ledger)

"히스토리가 너무 길다면 분할할 수 있는 방법은 없을까" 라는 Owner 질문. 재 보니
v1.59.0 에서 손댄 PROCESS(1671줄)보다 **훨씬 큰 것이 있었다.**

```
DECISIONS.md  5424줄 338KB  절 208개   ← critic 이 매 단계 통째로 받는다
DESIGN.md     4880줄 229KB  절  24개
PROCESS.md    1671줄  95KB  절  79개
BACKLOG.md     891줄 119KB
DIRECTION.md   481줄  30KB
```

**`BACKLOG_DONE.md` 는 하네스에 없는 파일인데 팀이 스스로 만들어 쓰고 있었다.** 분할 필요가
실재한다는 증거다. 관례를 정식화했다.
...
| 축 | 판정 |
|---|---|
| **브랜치별** | **기각.** 단계 브랜치는 main 에 합쳐져 사라진다. "상태가 전부 파일에 있다"는 전제(무손실 모델 전환)가 깨진다 |
| 모듈·파일 단위 | 부분적. 결정은 대개 교차 관심사라 축이 못 된다 |
| **활성 / 아카이브** | **채택.** 팀이 이미 그렇게 쪼갰고, 모든 문서에 같은 규칙으로 적용된다 |

### 최근 N단계만 자르면 안 된다

`--keep 3` 이면 5424줄이 634줄이 된다. 그런데 stage-2 의 "sqlite 를 쓴다" 같은 기초 결정이
창 밖으로 나가면 critic 이 모순을 못 보고 통과시킨다 — **못 본 것을 "충돌 없음" 으로 바꾸는
것이라 원칙 1 위반이다.** 그래서 ADR 방식으로 간다:

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| critic 이 받는 DECISIONS | **전문 5424줄** | **전체 제목 208건 + 최근 N단계 본문** (`--keep 3` 에서 634줄) |
| 옛 결정 | 통째로 들어감 | **제목은 언제나 보인다.** 본문이 필요하면 원문을 읽는다 |
| 단계 번호를 못 읽는 절 | — | **자르지 않는다**(43건이 그랬다) |
| 완료된 백로그 | BACKLOG 에 누적 | **`BACKLOG_DONE.md` 로 옮긴다**(12b-1). 지우지 않는다 |
| 원본 | — | **한 줄도 자르지 않는다** |

`tests/test_selfcheck.py` 11항목 추가(97항목). `--ledger-stats` 로 크기를 언제든 본다.

## [1.59.0] — 소비자 없는 역할을 없애고, 누적 문서를 주입에서 떼어 낸다

"에이전트를 추가했더니 개발 속도와 효율이 낮아진 것 같다" 는 Owner 지적에서 시작했다.
실제 프로젝트를 재 보니 **원인이 에이전트 수가 아니었다.**

단계 하나에 LLM 호출이 최소 11회다. 그런데 각 역할이 무엇을 바꿨는지 세면:

| 역할 | 실측 | |
|---|---|---|
| **evaluator** | `SCORE.json` 5/5 `pass`·`below: []`·`delta 0`. 게다가 **GAP 출력을 쓰는 곳이 절차에 없다** — 재시도는 전부 `[score]` 가 몬다 | **폐지** |
| critic | DECISIONS 117회, 커밋 제목 9회(`critic 이 planner 의 중심 결정을 뒤집었다`) | 유지 |
| reviewer | 리뷰지적 단계당 9~41건 | 유지 |
| security | DECISIONS 132회, 커밋 21회(PII) | 유지 |

evaluator 는 "효과가 없다" 가 아니라 **효과를 가질 경로가 없었다.** 소비자가 없는 호출이다.

**진짜 병목은 누적 문서였다.**

```
PROCESS.md   P-규칙 76개 1558줄 · 그중 "대상: 전체" 1062줄
             → coder 를 한 번 부를 때마다 1200줄 넘게 프롬프트에 붙는다
DIRECTION.md 481줄(53절) — lead·critic 호출마다
```

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 역할에 주는 PROCESS | **전문** | `--process-active <역할>` (폐기 제외 + 대상 필터) |
| 역할에 주는 DIRECTION | **전문** | `--direction-head` (마지막 절만. 481 → 82줄) |
| 파일 자체 | — | **자르지 않는다.** append-only 이력 그대로 |
| 쌓인 것을 줄이는 법 | 없음 | **통합**. 새 P-번호가 옛 것을 "대체한다"고 적으면 원문은 남는다 |
| 쌓인 양 | 아무도 안 봄 | `--process-stats` 가 세고, 권고(300줄)를 넘으면 통합을 요구한다 |

**히스토리는 한 줄도 버리지 않는다.** 상한을 걸어 규칙을 떨어뜨리자는 첫 안은 Owner 지적으로
기각했다 — append-only 인 이유가 "왜 이 규칙이 생겼나" 를 남기기 위해서다. 보관과 주입을
나누는 것이 맞다.

다만 파생 주입만으로는 3%밖에 안 준다(폐기된 것이 76개 중 2개뿐). **통합이 76개 중 두 번밖에
안 일어난 것이 진짜 원인**이고, 메커니즘은 있었는데 안 쓰였다. 기계가 할 수 있는 것은
"지금 1200줄이 붙고 있다, 통합하라" 를 계속 보이게 만드는 것까지다.

폐지한 역할을 치우면서 **치우기 자체의 결함도 나왔다** — 치우기가 에이전트 블록 *안*에 있어서
지금 설치하는 오버레이만 청소했다. 예전에 codex 로 깔았다가 뺀 `.agents/skills/<role>` 은
영영 남는다. 블록 밖으로 뺐다. `ROLES_ALL` 에는 evaluator 를 **남긴다** — 빼면 옛 설치본에서
치울 수 없다.

`tests/test_selfcheck.py` 18항목 추가(86항목), `tests/test_install.py` 9항목 추가(111항목).

## [1.58.0] — 회고를 매 단계 돌리지 않는다 (트리거를 lead 에서 기계로)

Owner 질문에서 시작했다. "회고하는 게 lead 인데 lead 가 계획을 세우니 의미 없는 회고 아닌가."
파고 보니 절반 맞았고, 맞는 절반이 정확히 사각지대였다.

lead 는 `PLAN.json` 을 쓰지 않는다(planner 가 쓴다). 회고가 보는 6가지 신호도 전부 남의
실행 결과다. **그런데 그 신호의 원인 중 하나가 lead 자신의 결정**(DIRECTION·단계 초점)인데,
신호 목록에도 IMPROVE 대상에도 자기 자신이 없었고, 12c 증거 블록은 이번 단계 초점조차 주지
않았다. 자기 결정을 돌아볼 입력이 없었다.

그리고 12c 는 **매 단계 무조건** LLM 회고를 돌렸다. 역할 문서 자신이 "신호 없으면 제안하지
않는다" 고 적어 두었으니 대부분의 정답은 "없음" 이다. 실제 프로젝트는 101단계였다.

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 매 단계(12c) | **LLM 회고 1회 무조건** | **`--retro-check` 기계 판정.** 걸렸을 때만 회고 |
| 문턱 | 없음(lead 가 신호를 스스로 판단) | 재시도 ≥ 3 또는 리뷰지적 2단계 연속 증가 |
| "원인이 lead 결정인가" | 아무도 안 봄 | **critic 이 판정**(CAUSE) — 결정을 안 내린 쪽이 한다 |
| 증거 블록 | 남의 실행 결과만 | + 바뀐 파일 수 · 이번 단계 초점 · 적용 중인 P-번호 |
| 채택된 P-번호 | 쌓기만 함 | 17b 에서 **효과가 있었는지** 본다. 없으면 폐기 제안 |
| 최종 회고(17b) | 1회 | 그대로 |

**문턱은 실측으로 골랐다.** 처음에 제안한 절대값(재시도≥2 또는 리뷰지적≥3)을 실제 프로젝트
8단계에 대 보니 **8/8 전부 걸렸다** — 그 프로젝트는 단계당 리뷰지적 9~41건이 정상이라
절대값 문턱은 아무것도 거르지 못한다. 회고가 찾는 것은 "숫자가 크다" 가 아니라 얼마나 헤맸나다.

**파일 수 문턱도 실측으로 기각했다**(Owner 제안). 가장 나빴던 단계 201(재시도 5회·리뷰지적
41건·그 단계에서만 절차개선 P-57~P-66 열 건)이 **제품코드 3개**로 가장 작았다. 파일 수로
걸면 회고가 가장 필요한 단계를 놓친다. `feature냐 버그수정이냐` 축도 못 가른다 — 나빴던 둘
중 하나는 기능(198), 하나는 수정(201)이었다. 파일 수는 회고가 돌 때 맥락으로만 준다.

최종 문턱으로 같은 8단계를 재생하면 **198·201 두 단계에서만** 걸린다. 호출이 1/4로 준다.

기록이 없으면 YES 다 — 실제 프로젝트 TEST_LOG 110행 중 102행이 `-` 였다(7열 도입 전).
못 읽은 것을 "문제 없음" 으로 바꾸지 않는다.

`tests/test_selfcheck.py` 12항목 추가(68항목) — 실측 8단계 재생을 그대로 테스트에 넣었다.

## [1.57.0] — 동결이면 설치 중에 묻는다

"사용자들이 `--accept-constitution` 을 따로 돌리지 않는데 기본으로 넣으면 어떤가" 라는
Owner 질문에서 나왔다. 지적은 맞다 — 안 돌리면 merge 가 영영 막히니 덫이다. 그런데 **기본
채택은 안 된다.** 바로 앞 v1.55~1.56 에서 본 것이 근거다:

- 이관 판정은 어절 겹침 **어림짐작**이라 실제 프로젝트에서 12줄이 사람 판단을 필요로 했다.
- 그중 `Stage-Gate 는 쓰지 않는다` 는 새 헌법과 **정면으로 충돌**한다 — 자동 활성화하면 안 된다.
- 157줄짜리 프로젝트 정체성 문서의 성격이 "헌법" 에서 "좁히는 규칙" 으로 조용히 바뀐다.

남의 문서를 묻지 않고 고쳐 쓰는 것이 이 저장소가 거듭 고친 실패 유형이다. 그래서 **묻는다.**

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 터미널에서 실행 | 안내만, Owner 가 명령을 다시 침 | **그 자리에서 3지선다** (Gate 0·1 과 같은 방식) |
| headless·CI | 동일 | **보존**(무인 실행이 헌법을 고치면 안 된다) |
| 플래그 | `--accept-constitution` | 그대로 — 자동화용 |
| 답이 없으면(EOF) | — | **2번(그대로)** — 안전한 쪽 |

1번을 고르면 `--accept-constitution` 으로 설치를 한 번 더 돌린다. 플래그로 돌린 것과 결과가
같아야 하므로 다른 경로를 만들지 않는다.

`HARNESS_FORCE_PROMPT` 는 테스트용 문이다. **묻게** 만들 뿐이고 답은 여전히 stdin 에서
와야 하며, 답이 없으면 보존이라 이 변수만으로 헌법이 바뀌는 일은 없다.

`tests/test_install.py` 에 17항목 추가(102항목) — 네 갈래(사람 없음·2·3·EOF)와 1번까지 전부.
`verify_parity.py` 22항목.

## [1.56.0] — 어림짐작에 걸린 줄을 조용히 버리던 것

v1.55.0 으로 다시 옮기고 세어 보니 97줄에서 11줄로 줄었다. 남은 11줄을 읽어 보니 **진짜
Owner 규칙**이 섞여 있었다 — `repos/ 에서는 어디로도 push 하지 않는다`,
`repos/ 에는 브랜치를 만들지 않는다`, Windows 셸 제약 같은 것들이다.

Owner 줄인지 하네스 옛 문장인지는 어절 겹침으로 어림잡는다. 그 어림짐작은 **양쪽으로 틀린다**:

| | 전부 옮기면 | 걸린 것을 버리면 |
|---|---|---|
| 무슨 일이 나나 | `Stage-Gate 는 쓰지 않는다` 같은 **새 헌법과 충돌하는 옛 줄이 규칙으로 되살아난다** | 진짜 Owner 규칙이 **말없이 사라진다** |

둘 다 틀렸으므로 어느 쪽도 고르지 않는다 — **활성화하지 않되 버리지도 않는다.**

| | 지금(AS-IS) | 앞으로(TO-BE) |
|---|---|---|
| 애매한 줄 | 조용히 누락 | `--owner-lines-skipped` 로 내어 **설치기가 보고** |
| Owner 가 보는 것 | 아무것도 | 몇 줄인지 + 앞 8줄 + 원문 위치(`.owner-backup`) |
| 고르는 주체 | 어림짐작 | **Owner** |

`tests/test_selfcheck.py` 6항목(56항목), `tests/test_install.py` 3항목(85항목).
설치 테스트의 미끼 줄은 **실제 렌더된 헌법에서 특징 어절을 뽑아 만든다** — 지어낸 줄은
어림짐작에 안 걸려 테스트가 헛돈다(처음에 그렇게 써서 통과해 버렸다).

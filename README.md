...
## 이미 개발 중인 프로젝트에 설치할 때

빈 폴더가 아니라 **이미 코드가 있는 프로젝트**에도 그대로 설치한다. 다만 처음 한 번은
이런 모습이 된다 — 고장이 아니다.

| 보이는 것 | 뜻 | 할 일 |
|---|---|---|
| `WARN: selfcheck --gate 가 통과하지 않는다` | `--gate` 는 단계를 merge 할 때 쓰는 검사다. 기존 코드에 지적이 나는 게 정상이다 | "개발 시작" 으로 0단계 인터뷰를 하면 R번호·계획이 생기고 기준이 잡힌다 |
| `[lang] 감지된 제품 언어 없음` | 마커 파일(`requirements.txt`/`go.mod`/`Cargo.toml`/`package.json`)이 없거나 지원 언어가 아니다 | 지원 언어면 마커를 두고, 아니면 그대로 둔다(테스트 실행·판정은 checker가 제품 러너로 한다) |
| `알림: common/logger.py 가 이미 있어 덮지 않았습니다` | 쓰던 로거를 지키느라 건드리지 않았다 | 로그 형식만 맞으면 그대로 쓴다 |
| 기존 코드의 `print` 지적이 많다 | 한 번에 고칠 일이 아니다 | BACKLOG "메모·주의"에 쌓고 단계마다 줄인다 |

**Owner의 파일은 덮지 않는다** — `README.md`, `tests/`, `logs/`, `src/`, `common/logger.py`
모두 있으면 그대로 둔다. 새로 깔리는 것은 `AGENTS.md`·`dev-agent-team/`·에이전트 오버레이다.
...
1. `./init.sh --profile small --agent claude,opencode /tmp/t1` 과 `--profile large /tmp/t2` 실행,
   **설치 검증 FAIL 0** 확인 (init.sh가 끝에서 `tests/verify_install.sh` 를 자동 수행).
   구성·파일·설정 파싱·헌법 동결을 보고, 가드 훅 25항목도 그 안에서 돈다(그중 3개는 guard.js 를
...
- **산출물을 점수로 재고, 그 점수로 재시도할지 정한다.** `selfcheck.py --score` 가 코드와
  ...
  `escalate`는 **직전이 이미 재시도였을 때만** 난다(새 결함의 첫 발견과 고쳐도 안 낫는 것은
  다르다). `size`는 근사치라 점수만 내고 재시도를 강제하지 않는다 — `--gate`가 안 막는 것과
  같은 이유다.
...
- **헌법이 옛 버전에 묶이면(동결) 그 사실이 보인다.** 재설치는 Owner가 고친 `AGENTS.md`를
  ...
  **설치를 한 번 더** 돌리는 것이다.
- **"개발 시작"은 신규·재개·업데이트 후 모두 같은 말이다.** team-dev의 "어디서부터 할지
  정한다"가 파일만 보고 진입점을 고른다(요구사항 없음→인터뷰 / 계획 없음→요구사항 확인 /
  진행 중→**현재 단계 이어가기** / 계획만 있고 미승인→Gate 1). **계획이 있으면 planner를
  다시 부르지 않는다** — 다시 세우면 진행한 단계가 날아간다. 업데이트로 절차가 바뀌어도
  새 절차로 이어가고, merge 게이트는 새 기준으로 돈다.

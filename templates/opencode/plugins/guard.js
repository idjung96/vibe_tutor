...
// 1) team/OWNER_QUESTION.md 에 "답: 번호"가 없으면 모든 도구 사용을 막는다.
...
export const TeamGuard = async ({ directory }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const q = path.join(root, "team", "OWNER_QUESTION.md");
      if (fs.existsSync(q)) {
        const txt = fs.readFileSync(q, "utf8");
        if (!/^답:\s*[0-9]/m.test(txt)) {
          throw new Error(
            "Owner 답변 대기 중: team/OWNER_QUESTION.md의 '답:'에 번호가 적힐 때까지 멈춘다."
          );
        }
      }
      // 2. 기존 테스트 파일 보호
